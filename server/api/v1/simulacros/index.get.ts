...
export default defineEventHandler(async (event) => {
  try {
    ...
    const total = await Simulacro.countDocuments(filter)
    const simulacros = await Simulacro.find(filter).sort({ fecha: -1 }).skip((page - 1) * limit).limit(limit)
    return { success: true, data: simulacros, pagination: { page, limit, total, pages: Math.ceil(total / limit) } }
